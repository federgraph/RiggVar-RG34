...
namespace RiggVar.Compat
{
    ...
    public abstract class TStrings : ICloneable, IEnumerable, ICollection
    {
        ...
        {
            Stream s = new FileStream(AFileName, FileMode.Open, FileAccess.Read, FileShare.Read);
            using (s)
            {
                LoadFromStream(s);
            }
        ...
        public virtual void LoadFromStream(System.IO.Stream AStream)
        {
            ...
            try
            {
                StreamReader sr = new StreamReader(AStream, Encoding.UTF8);
                using (sr)
                {
        ...
        {
            Stream s = new FileStream(AFileName, FileMode.Create, FileAccess.Write, FileShare.None);
            using (s)
            {
                SaveToStream(s);
            }
        ...
        {
            StreamWriter sw = new StreamWriter(AStream, Encoding.UTF8);
            using (sw)
            {
                sw.Write(GetTextStr());
            }
