...
namespace RiggVar.Rgg
{
    ...
    public class TRiggFS : TMast
    {
        ...
        public int[] iPe = new int[Rigg.TIntRiggPoints]; // Integerkoordinaten entlastet 3d in mm
        ...
        protected virtual void Kraefte()
        {
            ...
            // neue Geometrie
            Fachwerk.KX[fw1] = rP[Rigg.ooE].X; // Angriffspunkt Mastcontroller 
            Fachwerk.KY[fw1] = rP[Rigg.ooE].Z;
            Fachwerk.KX[fw2] = rP[Rigg.ooA].X; // Saling 
            Fachwerk.KY[fw2] = rP[Rigg.ooA].Z;
            Fachwerk.KX[fw3] = rP[Rigg.ooC].X; // Angriffspunkt Wante 
            Fachwerk.KY[fw3] = rP[Rigg.ooC].Z;

            Fachwerk.KX[fw4] = rP[Rigg.ooC0].X;
            Fachwerk.KY[fw4] = rP[Rigg.ooC0].Z;
            Fachwerk.KX[fw5] = rP[Rigg.ooD0].X;
            Fachwerk.KY[fw5] = rP[Rigg.ooD0].Z;
            Fachwerk.KX[fw6] = rP[Rigg.ooA0].X;
            Fachwerk.KY[fw6] = rP[Rigg.ooA0].Z;
        ...
        {
            double P0P, PC, PD, P0C0, P0D0;

            // Längen bereitstellen:
            ...
            // Punkte A, B  
            SplitF.h = P0P;
            SplitF.l2 = rL.A0B0 - rL.AB; // PüttingAbstand - SalingAbstand 
            ...
            for (int j = 0; j <= 19; j++)
            {
                if (Math.Abs(rF[j]) > 32000)
                {
                    ...
                    LogList.AppendLine(string.Format("TRiggFS.Split: Betrag rF[{0}] > 32000 N", j));
                }
            }
        }
        ...
        protected virtual void KraefteOS()
        {
            ...
            {
                temp = Math.Sqrt(Sqr(rL[8] + rL[13]) - Sqr(rL[6] / 2));
                temp = Math.Atan(rL[6] / 2 / temp);
        ...
        protected void Probe()
        {
            ...
            if ((SalingTyp == TSalingTyp.stFest) || (SalingTyp == TSalingTyp.stDrehbar))
            {
                ...
                LogList.AppendLine(string.Format("TRiggFS.Probe: Probe A0 = {0}", tempResult)); // %6.2f
                test = test && temptest;
                // Probe Punkt B0  
                temptest = Probe(Rigg.ooB0, Rigg.ooA0, Rigg.ooB, Rigg.ooC0, Rigg.ooD0, 6, 7, 2, 4);
                tempResult = TetraF.ProbeErgebnis;
                LogList.AppendLine(string.Format("TRiggFS.Probe: Probe B0 = {0}", tempResult));
                test = test && temptest;
                // Probe Punkt C0  
                ...
                tempResult = TetraF.ProbeErgebnis;
                LogList.AppendLine(string.Format("TRiggFS.Probe: Probe C0 = {0}", tempResult));
                ...
                tempResult = TetraF.ProbeErgebnis;
                LogList.AppendLine(string.Format("TRiggFS.Probe: Probe D0 = {0}", tempResult));
                TetraF.KnotenLast = Vcalc.Null;
                test = test && temptest;
                // Probe Punkt A  
                temptest = Probe(Rigg.ooA, Rigg.ooA0, Rigg.ooB, Rigg.ooC, Rigg.ooD, 8, 11, 13, 10);
                tempResult = TetraF.ProbeErgebnis;
                LogList.AppendLine(string.Format("TRiggFS.Probe: Probe A = {0}", tempResult));
                test = test && temptest;
                // Probe Punkt B  
                temptest = Probe(Rigg.ooB, Rigg.ooA, Rigg.ooB0, Rigg.ooC, Rigg.ooD, 11, 7, 12, 9);
                tempResult = TetraF.ProbeErgebnis;
                LogList.AppendLine(string.Format("TRiggFS.Probe: Probe B = {0}", tempResult));
                test = test && temptest;
                // Probe Punkt C  
                TetraF.KnotenLast = KnotenLastC;
                temptest = Probe(Rigg.ooC, Rigg.ooA, Rigg.ooB, Rigg.ooC0, Rigg.ooD0, 13, 12, 14, 0);
                tempResult = TetraF.ProbeErgebnis;
                LogList.AppendLine(string.Format("TRiggFS.Probe: Probe C = {0}", tempResult));
                TetraF.KnotenLast = Vcalc.Null;
                test = test && temptest;
                ...
                {
                    LogList.AppendLine("TRiggFS.Probe: Probe O.K.");
                }
            }
            ...
            if ((SalingTyp == TSalingTyp.stOhneStarr) || (SalingTyp == TSalingTyp.stOhneBiegt))
            {
                ...
                temptest = Probe(Rigg.ooA0, Rigg.ooA, Rigg.ooB0, Rigg.ooC0, Rigg.ooD0, 8, 6, 3, 5);
                tempResult = TetraF.ProbeErgebnis;
                LogList.AppendLine(string.Format("TRiggFS.Probe: ProbeOS A0 = {0}", tempResult));
                test = test && temptest;
                // Probe Punkt B0  
                ...
                temptest = Probe(Rigg.ooC0, Rigg.ooA0, Rigg.ooB0, Rigg.ooD0, Rigg.ooC, 3, 2, 1, 14);
                tempResult = TetraF.ProbeErgebnis;
                LogList.AppendLine(string.Format("TRiggFS.Probe: ProbeOS C0 = {0}", tempResult));
                TetraF.KnotenLast = Vcalc.Null;
                test = test && temptest;
                // Probe Punkt D0  
                TetraF.KnotenLast = KnotenLastD0;
                temptest = Probe(Rigg.ooD0, Rigg.ooA0, Rigg.ooB0, Rigg.ooC0, Rigg.ooC, 5, 4, 1, 0);
                tempResult = TetraF.ProbeErgebnis;
                LogList.AppendLine(string.Format("TRiggFS.Probe: ProbeOS D0 = {0}", tempResult));
                TetraF.KnotenLast = Vcalc.Null;
                test = test && temptest;
                // Probe Punkt C  
                TetraF.KnotenLast = KnotenLastC;
                temptest = Probe(Rigg.ooC, Rigg.ooA, Rigg.ooB, Rigg.ooC0, Rigg.ooD0, 13, 12, 14, 0);
                tempResult = TetraF.ProbeErgebnis;
                LogList.AppendLine(string.Format("TRiggFS.Probe: ProbeOS C = {0}", tempResult));
                TetraF.KnotenLast = Vcalc.Null;
                test = test && temptest;
                ...
                {
                    LogList.AppendLine("TRigg.Probe: Probe O.K.");
                }
            }
        }
        ...
        public int Regeln(TTrimm TrimmSoll)
        {
            ...
            switch (CalcTyp)
            {
                ...
                case TCalcTyp.ctQuerKraftBiegung:
                    //limitA = limitA; // in mm, wird in BiegeUndNeige ermittelt  
                ...
                case TCalcTyp.ctKraftGemessen:
                    ...
                    return 0; // weil Kraft schon bekannt 					
